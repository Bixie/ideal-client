	...
	*/
	...
	class IdealRequest
	{
		...
		protected function getSignature($sMessage, $sKeyName, $sKeyPassword = false)
		{
			...
			$sql = "SELECT `file_content` FROM `" . $aDatabaseSettings['prefix'] . "idealcheckout_ssl` WHERE (`file_name` = '" . idealcheckout_escapeSql($sKeyName) . "') LIMIT 1;";
		...
		protected function getCertificateFingerprint($sCertificateName)
		{
			...
			$sql = "SELECT `file_content` FROM `" . $aDatabaseSettings['prefix'] . "idealcheckout_ssl` WHERE (`file_name` = '" . idealcheckout_escapeSql($sCertificateName) . "') LIMIT 1;";
