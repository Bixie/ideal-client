<?php
	...
	class Gateway extends GatewayCore
	{
		...
		// Setup payment
		public function doSetup($sOrderId, $sOrderCode)
		{
			...
			if(empty($sOrderId) || empty($sOrderCode))
			{
...
<form action="https://www.ideal-checkout.nl/payment/" method="post">
	...
	<input name="url_success" type="hidden" value="' . htmlspecialchars(idealcheckout_getRootUrl(1) . 'index.php?task=return&transaction_id=' . $this->oRecord['transaction_id'] . '&transaction_code=' . $this->oRecord['transaction_code'] . '&status=SUCCESS') . '">
	<input name="url_pending" type="hidden" value="' . htmlspecialchars(idealcheckout_getRootUrl(1) . 'index.php?task=return&transaction_id=' . $this->oRecord['transaction_id'] . '&transaction_code=' . $this->oRecord['transaction_code'] . '&status=PENDING') . '">
	<input name="url_cancel" type="hidden" value="' . htmlspecialchars(idealcheckout_getRootUrl(1) . 'index.php?task=return&transaction_id=' . $this->oRecord['transaction_id'] . '&transaction_code=' . $this->oRecord['transaction_code'] . '&status=CANCELLED') . '">
	<input name="url_error" type="hidden" value="' . htmlspecialchars(idealcheckout_getRootUrl(1) . 'index.php?task=return&transaction_id=' . $this->oRecord['transaction_id'] . '&transaction_code=' . $this->oRecord['transaction_code'] . '&status=FAILURE') . '">
	<input type="submit" value="Verder >>">
		...
		// Catch return
		public function doReturn($client)
		{
			$sHtml = '';
			...
			else
			{
				...
				if($this->getRecordByTransaction())
				{
					...
					if($bStatusChanged)
					{
						...
						$client->updateStatus($this->oRecord['transaction_id'], $this->oRecord['transaction_code']);
					...
					elseif(strcasecmp($this->oRecord['transaction_status'], 'CANCELLED') === 0)
					{
						...
						$sHtml .= '<p>Uw betaling is geannuleerd. Probeer opnieuw te betalen.<br><input style="margin: 6px;" type="button" value="Verder" onclick="javascript: document.location.href = \'' . htmlspecialchars(idealcheckout_getRootUrl(1) . 'index.php?task=setup&order_id=' . $this->oRecord['order_id'] . '&order_code=' . $this->oRecord['order_code']) . '\'"></p>';
					}
